    ...
    def define(self):
        ...
        if eval_pts_option == 'auto':
            ...
            for i in range(len(surface_names)):
                ...
                self.register_output(f'{surface_names[i]}_circulations', circulations[:, start:start + delta])
                self.register_output(f'{surface_names[i]}_gamma_b', gamma_b[:, start:start + delta])
...
class ThrustDragUndynamic(Model):
    """
    L,D,cl,cd
    parameters
    ----------
